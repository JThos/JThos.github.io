...
// Add to window object for skills to access
window.azureAPIKey = azureAPIKey;
window.azureEndpoint = azureEndpoint;
window.azureAPIVersion = azureAPIVersion;
  ...
  // Make available to skills via window object
  window.azureAPIKey = azureAPIKey;
  window.azureEndpoint = azureEndpoint;
  window.azureAPIVersion = azureAPIVersion;
 ...
 function extractGraphPropertiesCSV() {
  ...
  try {
    ...
    console.log("Node regular attributes:", regularAttributes);
    console.log("Node sparse attributes:", sparseAttributes);
    ...
    console.log("Edge regular attributes:", edgeRegularAttributes);
    console.log("Edge sparse attributes:", edgeSparseAttributes);
...
function generateSystemPrompt() {
  ...
  const systemPrompt = `
You are an assistant that analyzes Piping and Instrumentation Diagrams (P&IDs). Your task is to answer questions about the P&ID graph data provided. You MUST ONLY answer questions related to P&IDs, process engineering, and the specific diagram data provided. For any other questions, politely explain that you're specialized in P&ID analysis and offer to help with diagram-related queries instead.
...
1. The node and edge data includes regular columns and may contain a "sparse" column that consolidates infrequently populated attributes in "key: value" pairs separated by semicolons.
2. When analyzing this data, examine both the regular columns AND parse any sparse column values to extract all relevant attributes.
3. Every node has a unique ID that can be used for graph construction and path finding.
4. Every edge has a start node, end node, and attributes that describe the connection.

ITEM TAG HANDLING:
1. Users frequently refer to equipment using partial item tags (e.g., "VES-301" instead of the full "1011-VES-301" where "1011" is the unit number prefix).
2. When a user mentions an item tag, ALWAYS:
   - First try exact matching on the ItemTag field
   - If no exact match, search for PARTIAL matches where the user's tag appears within the full tag
   - Be flexible with separators (users might type "VES301" or "VES 301" instead of "VES-301")
   - Check both regular tag fields and any tags that might appear in the sparse column
3. When showing results, include the full item tag in your response for clarity.

CONNECTIVITY UNDERSTANDING:
1. When users ask if components are "connected" (e.g., "Is VES-301 connected to VES-5003?"), they are almost always asking about INDIRECT connections through the process flow paths, not just direct connections.
2. Always interpret connectivity questions as asking for ANY possible flow path between components, not just immediate neighbors.
3. Use appropriate path-finding algorithms to discover these indirect connections.
4. A "connection" may involve multiple intermediate components and should consider the entire path.

RECOMMENDED GRAPH HANDLING:
When using networkx to analyze this data:
- Create a directed graph (DiGraph) with NODEID values as networkx Node IDs
- Use EDGEID values as edge IDs in edge attributes
- Load all regular attributes as node/edge properties
- Parse the "sparse" column to extract additional attributes (split by semicolons, then by ":" to get key-value pairs)
- Use appropriate networkx functions like shortest_path(), all_simple_paths(), node_connected_component() etc.

COMMON QUERIES AND HOW TO HANDLE THEM:

1. Path Finding:
   - For "Find path between X and Y" queries, use networkx.shortest_path() or networkx.all_simple_paths()
   - Always include ALL nodes and edges along the path in your response
   - Consider flow direction when relevant (some pipes may be directional)
   - Leverage existing functions like FindDownstreamPaths() and IsolateEquipment() where appropriate

2. Equipment Queries:
   - For "Show all valves" or "Count pumps" queries, search ALL attributes including sparse column values
   - Check both type and subtype fields, and any other relevant attributes
   - Return complete lists of matching elements

3. Isolation Queries:
   - For "Isolate equipment X" queries, identify all valves that would need to be closed
   - Consider both upstream and downstream connections
   - Include all relevant isolation points in your response

4. Flow Analysis:
   - Analyze potential flow paths considering the direction attributes of edges
   - Identify potential bottlenecks or critical paths
   - Consider how different valve states affect flow possibilities

5. Connectivity Queries:
   - For "Is X connected to Y" queries, search for ANY path between the components
   - Consider both direct and indirect connections through multiple components
   - Report all intermediate nodes along the connection path
   - Explain the nature of the connection (e.g., "VES-301 is connected to VES-5003 through valve V-421 and pump P-105")

You MUST respond in JSON format with the following structure:
{
  "response": {
    "answer": "Your detailed explanation goes here. Provide a complete, thorough answer that addresses every aspect of the query.",
    "highlight": {
...
CRITICAL GUIDELINES:
1. THINK STEP-BY-STEP about the user's query before answering.
2. SEARCH THOROUGHLY through all attributes, including sparse column values.
3. ALWAYS include ALL relevant nodes and edges in the highlight section.
4. NEVER respond with "I cannot do this" without first trying multiple approaches:
   - Try different search strategies (exact match, substring, pattern matching)
   - Consider both directed and undirected graph analysis
   - Look at both direct properties and derived/calculated properties
   - Handle partial item tags by searching for substrings within full tags
5. COMPLETE YOUR ANSWERS fully - never truncate your response or leave an analysis unfinished.
6. For path queries, ALWAYS return ALL nodes and ALL edges along the path.
7. For connectivity queries, ALWAYS assume the user is asking about INDIRECT connections through the process flow, not just direct connections.
8. Consider indirect connections of reasonable range (at least 5% of node count in graph) when asked questions like "Is pump P-100 connected to a vessel" or "isolate heat exchanger H-100".
9. The visualization depends on proper highlighting, so be thorough with your highlight lists.

Remember: The quality of the visualization directly depends on your JSON response being complete and accurate. Users rely on your analysis to make important decisions about process systems. Ensure your "answer" field contains a comprehensive response that isn't cut off.
`;
...
# Process sparse attributes if they exist
if 'sparse' in nodes_df.columns:
    for _, row in nodes_df.iterrows():
        node_id = row['id']
        if pd.notna(row['sparse']) and row['sparse']:
            sparse_attrs = {}
            pairs = row['sparse'].split(';')
            for pair in pairs:
                if ':' in pair:
                    key, value = pair.split(':', 1)
                    sparse_attrs[key.strip()] = value.strip()
            nx.set_node_attributes(G, {node_id: sparse_attrs})

if 'sparse' in edges_df.columns:
    for _, row in edges_df.iterrows():
        edge_id = row['id']
        if pd.notna(row['sparse']) and row['sparse']:
            sparse_attrs = {}
            ...
            for pair in pairs:
                if ':' in pair:
                    key, value = pair.split(':', 1)
                    sparse_attrs[key.strip()] = value.strip()
            # Need to update edge attributes correctly
            start_node, end_node = row['startnode'], row['endnode']
            current_attrs = G[start_node][end_node]
            G[start_node][end_node].update(sparse_attrs)

# Function to find paths between components
def find_paths(start_node, end_node, max_paths=5):
    try:
        paths = list(nx.all_simple_paths(G, start_node, end_node, cutoff=15))
...
# Function to get all elements by type or subtype
def get_elements_by_type(element_type, is_subtype=False):
    matching_nodes = []
    
    # Check regular attributes
    attr_name = 'subtype' if is_subtype else 'type'
    
    # Search regular attributes
    for node, attrs in G.nodes(data=True):
        if attr_name in attrs and attrs[attr_name] and isinstance(attrs[attr_name], str):
            if element_type.lower() in attrs[attr_name].lower():
                matching_nodes.append(node)
        # Also search in sparse attributes that might contain type information
        for key, value in attrs.items():
            if isinstance(value, str) and element_type.lower() in value.lower():
                if key.lower().endswith('type') or 'type' in key.lower():
                    if node not in matching_nodes:
                        matching_nodes.append(node)
    
    return matching_nodes

# Function to find items by tag (full or partial)
def find_by_tag(tag_query):
    matching_nodes = []
    
    # Standardize the query - remove spaces and convert to lowercase
    clean_query = tag_query.lower().replace(' ', '')
    
    for node, attrs in G.nodes(data=True):
        # Check itemtag attribute
        if 'itemtag' in attrs and attrs['itemtag']:
            clean_tag = attrs['itemtag'].lower().replace(' ', '')
            if clean_query in clean_tag:
                matching_nodes.append(node)
                continue
        
        # Check sparse attributes for tags
        for key, value in attrs.items():
            if isinstance(value, str) and ('tag' in key.lower() or 'id' in key.lower() or 'name' in key.lower()):
                clean_value = value.lower().replace(' ', '')
                if clean_query in clean_value:
                    matching_nodes.append(node)
                    break
...
# Identify if this is a connectivity query
connectivity_terms = ['connected', 'connection', 'connects to', 'path', 'route']
is_connectivity_query = any(term in query for term in connectivity_terms)

# Identify if this is an equipment query
equipment_terms = ['valve', 'pump', 'vessel', 'tank', 'heat exchanger', 'compressor', 'filter']
is_equipment_query = any(term in query for term in equipment_terms)

# Identify if this is an isolation query
isolation_terms = ['isolate', 'isolation', 'shutdown', 'shut down', 'close']
is_isolation_query = any(term in query for term in isolation_terms)

# Extract equipment tags from query
import re
# Look for patterns like VES-301, P-100, TCV-5003, etc.
tag_pattern = r'\\b([A-Za-z]+[-\\s]?\\d+)\\b'
potential_tags = re.findall(tag_pattern, query)

# Path finding and connectivity queries
if is_connectivity_query and len(potential_tags) >= 2:
    # Find nodes matching the tags
    start_nodes = find_by_tag(potential_tags[0])
    end_nodes = find_by_tag(potential_tags[1])
    
    if start_nodes and end_nodes:
        # For simplicity, use the first match for each (could be enhanced)
        ...
        if paths:
            ...
            # Get item tags for better readability
            node_tags = {}
            for node in all_nodes:
                if 'itemtag' in G.nodes[node] and G.nodes[node]['itemtag']:
                    node_tags[node] = G.nodes[node]['itemtag']
                else:
                    node_tags[node] = f"Node {node}"
            
            # Format the path description
            path_desc = []
            for path in paths[:3]:  # Limit to first 3 paths
                path_str = " → ".join([node_tags.get(n, f"Node {n}") for n in path])
                path_desc.append(path_str)
            ...
            result = {
                ...
                "answer": f"I found {len(paths)} path(s) connecting {node_tags.get(start_node)} to {node_tags.get(end_node)}. " + 
                          f"The path(s) are: {'; '.join(path_desc)}."
            }
        else:
            result = {
                "paths_found": 0,
                "highlight": {"nodes": [], "edges": []},
                "answer": f"No path found between the specified components."
            }
    else:
        # Couldn't find one or both tags
        missing = []
        if not start_nodes:
            missing.append(potential_tags[0])
        if not end_nodes:
            missing.append(potential_tags[1])
        
        result = {
            "highlight": {"nodes": [], "edges": []},
            "answer": f"I couldn't find equipment with tag(s): {', '.join(missing)}. Please check the tag and try again."
        }

# Equipment queries (e.g., "show all valves")
elif is_equipment_query:
    for term in equipment_terms:
        if term in query:
            # Find elements by type/subtype
            matching_elements = get_elements_by_type(term, is_subtype=True)
            if not matching_elements:
                matching_elements = get_elements_by_type(term, is_subtype=False)
            
            # Get item tags for better descriptions
            element_details = []
            for elem in matching_elements:
                if 'itemtag' in G.nodes[elem] and G.nodes[elem]['itemtag']:
                    element_details.append(G.nodes[elem]['itemtag'])
                else:
                    element_details.append(f"Node {elem}")
            
            result = {
                "count": len(matching_elements),
                "highlight": {
                    "nodes": matching_elements,
                    "edges": []
                },
                "answer": f"I found {len(matching_elements)} {term}(s) in this P&ID: {', '.join(element_details[:15])}" + 
                          (f" and {len(element_details) - 15} more." if len(element_details) > 15 else ".")
            }
            break

# Isolation queries
elif is_isolation_query and potential_tags:
    # Find the element to isolate
    target_nodes = find_by_tag(potential_tags[0])
    
    if target_nodes:
        target_node = target_nodes[0]
        
        # Find valves upstream and downstream (simplified approach)
        # In a real scenario, you'd need more complex logic based on flow direction
        
        # Get connected components
        connected = nx.node_connected_component(G.to_undirected(), target_node)
        subgraph = G.subgraph(connected)
        
        # Find valves in this subgraph
        isolation_valves = []
        for node in subgraph:
            attrs = G.nodes[node]
            if ('subtype' in attrs and 'valve' in attrs['subtype'].lower()) or \
               ('type' in attrs and 'valve' in attrs['type'].lower()):
                isolation_valves.append(node)
        
        # Get item tags for better descriptions
        valve_details = []
        for valve in isolation_valves:
            if 'itemtag' in G.nodes[valve] and G.nodes[valve]['itemtag']:
                valve_details.append(G.nodes[valve]['itemtag'])
            else:
                valve_details.append(f"Valve {valve}")
        
        # Get tag of target
        target_tag = "the equipment"
        if 'itemtag' in G.nodes[target_node] and G.nodes[target_node]['itemtag']:
            target_tag = G.nodes[target_node]['itemtag']
        
        result = {
            "count": len(isolation_valves),
            "highlight": {
                "nodes": isolation_valves + [target_node],
                "edges": []
            },
            "answer": f"To isolate {target_tag}, you would need to close these valves: {', '.join(valve_details[:15])}" + 
                      (f" and {len(valve_details) - 15} more." if len(valve_details) > 15 else ".")
        }
    else:
        result = {
            "highlight": {"nodes": [], "edges": []},
            "answer": f"I couldn't find equipment with tag {potential_tags[0]}. Please check the tag and try again."
        }

# Default response if no specific query type was identified
if not result:
    # Create a general overview of the diagram
    ...
    for node, attrs in G.nodes(data=True):
        if 'subtype' in attrs and attrs['subtype']:
            equip_type = attrs['subtype']
            if equip_type in equipment_counts:
                equipment_counts[equip_type] += 1
            else:
                equipment_counts[equip_type] = 1
    ...
    if equipment_counts:
        ...
        equipment_summary = "Main equipment types: " + ", ".join([f"{t}: {c}" for t, c in top_equipment]) + "."
    
    result = {
        "highlight": {"nodes": [], "edges": []},
        "answer": f"This P&ID contains {total_nodes} nodes and {total_edges} connections. {equipment_summary} " +
                  "You can ask about specific equipment, connections between components, or request to highlight elements " +
                  "like 'Show all valves' or 'Find path from pump P-101 to vessel V-201'."
    }
...
print(json.dumps(output, indent=2))
    ...
    console.error("Detailed error in code interpreter mode:", error);
    // Log more context about the error
    console.error("Error context:", {
      queryLength: query?.length,
      nodeDataSample: nodeData?.substring(0, 100),
      edgeDataSample: edgeData?.substring(0, 100),
      errorName: error.name,
      errorStack: error.stack
    });
    
    return {
      text: "I encountered an error while analyzing the P&ID data. This could be due to complexity in the query or issue with the data format. Could you try rephrasing your question or asking about a specific part of the diagram?",
      highlightElements: []
    };
  }
}
