...
function setupContextMenu()
{
  ...
  // Ocultar el menú al hacer clic en cualquier lugar del documento
...
function FindUpstreamPaths(node1Id)
{
		...
		//upstreamNodeFilter="";
		//upwnstreamEdgeFilter="[@Type='Piping' or @Type='DirectConnection']";
		
		result = _findStreamPaths(node1Id, "Upstream", upstreamNodeFilter, upwnstreamEdgeFilter).sort((a,b) => a.length - b.length);
		if (result.length>0)
...
function IsolateEquipment(node1Id)
{
		...
		//isolateNodeFilter="/Data[contains(@Subtype,'valve')]";
